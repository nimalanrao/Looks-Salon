...
// --- Constants ---
const ASSETS = {
  logo: "/assets/looks salon logo.png",
  bannerLogo: "/assets/banner logo.png",
  hero: "/assets/hero.png",
  gallery: [
    "https://picsum.photos/seed/hair1/800/1000", // Before 1
    "https://picsum.photos/seed/hair2/800/1000", // After 1
    "https://picsum.photos/seed/hair3/800/1000", // Before 2
    "https://picsum.photos/seed/hair4/800/1000", // After 2
    "https://picsum.photos/seed/hair5/800/1000", // Before 3
    "https://picsum.photos/seed/hair6/800/1000"  // After 3
  ],
  brands: [
    "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9d/L%27Or%C3%A9al_logo.svg/1200px-L%27Or%C3%A9al_logo.svg.png",
    "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/L%27Oreal_Paris_logo.svg/2560px-L%27Oreal_Paris_logo.svg.png",
    "https://www.kerastase.com/-/media/project/loreal/brand-sites/kerastase/master/logos/kerastase-logo-black.png",
    "https://upload.wikimedia.org/wikipedia/commons/thumb/2/2f/Wella_logo.svg/1200px-Wella_logo.svg.png",
    "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a2/Dyson_logo.svg/1200px-Dyson_logo.svg.png"
  ],
  // --- POSTER SECTION ---
...
const BLOG_POSTS: BlogPost[] = [
  {
    ...
    image: "https://images.unsplash.com/photo-1531050171669-014464ce50e1?auto=format&fit=crop&q=80&w=800",
    content: (
...
const TESTIMONIALS: Testimonial[] = [
  {
    name: "Santha Kumari",
    rating: 5,
    text: "I started with eyebrow threading. The shape was perfect. The service was very good. Highly recommended for threading!",
    date: "2 weeks ago"
  },
  {
    name: "Ganesh MR",
    rating: 5,
    text: "Very good service... Fast service... Clean and neat work.... reasonable price compared to other shops around. Amazing experience!",
    date: "1 month ago"
  },
  {
    name: "Jatanthini Kathiravelu",
    rating: 5,
    text: "I had an amazing experience! The advice and guidance I received were so helpful, especially since I have sensitive skin. Highly recommend!",
    date: "3 days ago"
  }
];
...
const Navbar = ({ currentView, setView, activeCategory, setActiveCategory }: { currentView: View, setView: (v: View) => void, activeCategory: string, setActiveCategory: (c: string) => void }) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-700 ${isScrolled || currentView !== 'home' ? 'py-0 md:py-4' : 'py-0 md:py-8'}`}>
      <div className={`mx-auto flex justify-between items-center transition-all duration-700 ${isScrolled || currentView !== 'home' ? 'bg-black/90 backdrop-blur-xl md:rounded-full border-b md:border border-white/10 shadow-2xl py-4 md:py-3 px-6 md:px-10 md:mt-4 md:max-w-5xl w-full md:w-auto' : 'bg-transparent py-8 md:py-0 px-6 md:px-0 max-w-7xl w-full'}`}>
        <button 
...
const ServiceCard = ({ service, index }: { service: Service, index: number, key?: React.Key }) => (
  ...
  >
    ...
    <p className="text-zinc-500 text-sm leading-relaxed mb-8 opacity-80">{service.description}</p>
...
const ScrollMarquee = () => {
  ...
  // Reduced range for slower, ultra-smooth movement
  const x = useTransform(smoothProgress, [0, 1], ["-8000px", "-7000px"]);

  return (
    <div ref={ref} className="py-3 bg-zinc-950 overflow-hidden border-y border-white/5">
      <motion.div 
        style={{ x }}
        ...
        {[...Array(100)].map((_, i) => (
          <div key={i} className="flex items-center gap-12">
            <img 
              ...
              alt="LOOKS Salon Banner" 
              className="h-8 md:h-10 w-auto object-contain opacity-60 brightness-0 invert"
              referrerPolicy="no-referrer"
            />
...
const BrandMarquee = () => {
  const brands = [
    ASSETS.bannerLogo, ASSETS.bannerLogo, ASSETS.bannerLogo, ASSETS.bannerLogo,
    ASSETS.bannerLogo, ASSETS.bannerLogo, ASSETS.bannerLogo, ASSETS.bannerLogo
  ];
  ...
  return (
    <section className="py-24 bg-zinc-950 border-y border-white/5 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 mb-16 text-center">
        <span className="text-[10px] uppercase tracking-[0.8em] text-zinc-600 font-bold">In Partnership With</span>
      </div>
      ...
      <div className="relative flex overflow-hidden">
        <motion.div 
          ...
          className="flex items-center gap-48 md:gap-64 whitespace-nowrap px-12"
        >
          {brands.map((url, i) => (
            <div key={i} className="w-64 md:w-96 h-24 flex items-center justify-center opacity-70 hover:opacity-100 transition-all duration-700">
              <img 
                src={url} 
                alt="Brand Logo" 
...
const ReverseScrollMarquee = () => {
  ...
  // Reverse direction: moves from right to left as you scroll down
  const x = useTransform(smoothProgress, [0, 1], ["-7000px", "-8000px"]);
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <button 
              onClick={onClose}
              className="absolute top-4 right-4 z-30 w-10 h-10 bg-black/40 hover:bg-amber-600 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all shadow-lg"
            >
              <X className="w-6 h-6" />
            </button>
            ...
            {ASSETS.posters.length > 1 && (
              <>
                <button 
                  onClick={prevSlide}
                  className="absolute left-4 top-1/2 -translate-y-1/2 z-30 w-10 h-10 bg-black/40 hover:bg-amber-600 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all shadow-lg opacity-0 group-hover:opacity-100"
                >
                  <ChevronLeft className="w-6 h-6" />
                </button>
                <button 
                  onClick={nextSlide}
                  className="absolute right-4 top-1/2 -translate-y-1/2 z-30 w-10 h-10 bg-black/40 hover:bg-amber-600 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all shadow-lg opacity-0 group-hover:opacity-100"
                >
...
const ServicesView = ({ activeCategory, setActiveCategory }: { activeCategory: string, setActiveCategory: (c: string) => void }) => {
  ...
  const categories: ServiceCategory[] = [
    ...
    },
    {
      id: 'nails',
      title: 'Nail Services',
      image: "https://images.unsplash.com/photo-1604654894610-df490c817365?auto=format&fit=crop&q=80&w=1200",
      subsections: [
        {
    ...
    <div className="pt-32 pb-24 bg-ivory min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <BlurFade delay={0.4} className="mt-32 flex flex-col items-center gap-16">
          <div className="p-12 md:p-16 bg-white rounded-[48px] border border-zinc-100 shadow-2xl max-w-3xl w-full text-center relative overflow-hidden">
            ...
            <motion.a 
              ...
              whileTap={{ scale: 0.95 }}
              href="https://wa.me/60126175555"
              className="inline-flex items-center gap-4 px-16 py-6 bg-gold text-white rounded-full font-bold hover:bg-gold/90 transition-all shadow-2xl shadow-gold/30 group"
            >
          ...
          <div className="flex gap-12">
            <a href="https://twitter.com" className="text-zinc-400 hover:text-gold transition-all flex flex-col items-center gap-3 group">
              <div className="w-14 h-14 rounded-2xl bg-white border border-zinc-100 flex items-center justify-center group-hover:border-gold group-hover:bg-gold/5 transition-all">
                <Twitter className="w-6 h-6" />
              </div>
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold">Twitter</span>
            </a>
            <a href="https://wa.me/60126175555" className="text-zinc-400 hover:text-gold transition-all flex flex-col items-center gap-3 group">
              <div className="w-14 h-14 rounded-2xl bg-white border border-zinc-100 flex items-center justify-center group-hover:border-gold group-hover:bg-gold/5 transition-all">
                <MessageCircle className="w-6 h-6" />
              </div>
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold">Whatsapp</span>
            </a>
            <a href="https://maps.google.com/?q=Sentral+Suites+Brickfields" className="text-zinc-400 hover:text-gold transition-all flex flex-col items-center gap-3 group">
              <div className="w-14 h-14 rounded-2xl bg-white border border-zinc-100 flex items-center justify-center group-hover:border-gold group-hover:bg-gold/5 transition-all">
                <MapPin className="w-6 h-6" />
              </div>
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold">Google Maps</span>
            </a>
          </div>
        </BlurFade>
      </div>
          ...
          >
            ...
            <section id="gallery" className="py-20 md:py-32 bg-zinc-950 overflow-hidden">
              <div className="max-w-7xl mx-auto px-6 mb-12 md:mb-20">
                <BlurFade>
                  <SectionHeading title="Transformations" subtitle="The Gallery" light />
                </BlurFade>
              </div>
            ...
            <section className="py-20 md:py-32 bg-white">
              <div className="max-w-7xl mx-auto px-6">
                  ...
                  {TESTIMONIALS.map((t, i) => (
                    <BlurFade key={i} delay={i * 0.1}>
                      <motion.div 
                        ...
                        transition={{ delay: i * 0.1, duration: 0.8 }}
                        whileHover={{ y: -10 }}
                        className={`p-10 md:p-12 bg-ivory rounded-[32px] md:rounded-[48px] border border-zinc-50 hover:border-gold/20 transition-all group ${i === testimonialIndex ? 'block' : 'hidden md:block'}`}
                      >
                ...
                <div className="flex md:hidden justify-center items-center gap-6 mt-12">
                  <button 
                    onClick={() => setTestimonialIndex((prev) => (prev - 1 + TESTIMONIALS.length) % TESTIMONIALS.length)}
                    className="p-4 rounded-full border border-zinc-200 text-zinc-400 hover:text-gold hover:border-gold transition-all bg-transparent cursor-pointer"
                  >
                  ...
                  <div className="flex gap-2">
                    {TESTIMONIALS.map((_, i) => (
                      <div 
                        key={i} 
                        className={`w-1.5 h-1.5 rounded-full transition-all ${i === testimonialIndex ? 'bg-gold w-4' : 'bg-zinc-200'}`}
                      />
                    ))}
                  </div>
                  <button 
